angular.module('KramyaApp')
	.controller('RegisterCtlr', function($scope,$location, $window , $http , toaster,$timeout, $interval,anchorSmoothScroll,store , myService , jwtHelper) {
		...
		$scope.firstformData={'X-API-KEY': 'ANSHULVYAS16'};
		$scope.files=[];
      ...
      function(newValue){
		...
		if(store.get('jwt')){
			...
			if($scope.storeData.role=='hospital'){
				...
				var sendUrl="http://kramya.com/kramyaapi/index.php/hospitals?X-API-KEY="+$scope.apiKey+"&hospital_id="+id;
				$http({
					method: 'GET',
					url: sendUrl
				})
    ...
    $scope.firstFormSubmit=function(){
    	console.log('$scope.firstformData',$scope.firstformData);
    	var sendUrl='http://kramya.com/kramyaapi/index.php/hospitals';
			$http({
				method: 'POST',
				headers: {'Content-Type': 'application/json'},
				url: 'http://kramya.com/kramyaapi/index.php/hospitals/register_basic',
